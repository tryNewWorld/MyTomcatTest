...
public class UserDaoImpl extends BaseDao<UserBasic> implements UserDao {
    ...
    public List<UserBasic> queryFriendByUserId(Integer userId) {
        return super.executeQuery("select f.friend_id id, ub.name name from t_friend f left join t_user_basic ub on f.friend_id = ub.id where f.user_id = ?", userId.toString());
    }

    @Override
    public UserBasic getUserById(Integer id) {
        return super.load("select * from t_user_basic where id = ?",id.toString());
    }
}
